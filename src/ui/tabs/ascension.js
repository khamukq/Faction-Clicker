...
export const renderAscension = () => {
    ...
    const gain = Math.floor(Math.log1p(S.prestigePoints) * 0.5);
    ...
    container.innerHTML = `
        <div style="background:#0f0a08;border:2px solid #a855f7;border-radius:12px;padding:20px;margin:10px 0;text-align:center;">
            ...
            <div style="color:#a08060;margin:10px 0;">
                Уровень вознесения: <span style="color:#a855f7;font-weight:700;">${S.ascension}</span>
                <br>Очков вознесения: <span style="color:#a855f7;font-weight:700;">${S.ascensionPoints}</span>
            </div>
            <div style="color:#8a7a6a;margin:10px 0;font-size:14px;">
                Множитель силы: <span style="color:#34d399;">+${(S.ascension * 10).toFixed(0)}%</span>
                <br>Доступно очков: <span style="color:#a855f7;">${gain}</span>
                <br>Требуется престижа: <span style="color:#fbbf24;">5+ очков</span>
            </div>
            <button id="ascensionBtn" ${S.prestigePoints < 5 || gain <= 0 ? 'disabled' : ''}
                style="background:#6b3a4a;border:2px solid #a855f7;color:#a855f7;padding:10px 30px;border-radius:8px;cursor:${S.prestigePoints >= 5 && gain > 0 ? 'pointer' : 'not-allowed'};font-weight:700;font-size:16px;opacity:${S.prestigePoints >= 5 && gain > 0 ? 1 : 0.5};">
                ${S.prestigePoints >= 5 && gain > 0 ? '🌟 ВОЗНЕСТИСЬ' : '❌ НУЖНО 5+ ПРЕСТИЖА'}
            </button>
