    ...
    container.innerHTML = `
        <div style="background:#0f0a08;border:2px solid #f5c842;border-radius:12px;padding:20px;margin:10px 0;">
            <div style="text-align:center;">
                ...
                <h2 style="color:#f5c842;">${S.nickname}</h2>
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:10px;margin-top:20px;">
                <div class="stat-card">
                    ...
                    <div style="color:#f5c842;font-size:20px;font-weight:700;">${S.prestigePoints}</div>
                </div>
                <div class="stat-card">
                    <div style="color:#8a7a6a;font-size:11px;">✨ ВОЗНЕСЕНИЕ</div>
                    <div style="color:#a855f7;font-size:20px;font-weight:700;">${S.ascension}</div>
                </div>
                <div class="stat-card">
                    <div style="color:#8a7a6a;font-size:11px;">⚡ МНОЖИТЕЛЬ</div>
                    <div style="color:#34d399;font-size:20px;font-weight:700;">x${S.permanentMultiplier.toFixed(2)}</div>
                </div>
                <div class="stat-card">
                    <div style="color:#8a7a6a;font-size:11px;">🏗️ ЭТАЖ</div>
                    <div style="color:#60a5fa;font-size:20px;font-weight:700;">${S.floor}</div>
                </div>
                <div class="stat-card">
                    <div style="color:#8a7a6a;font-size:11px;">💀 УБИЙСТВ</div>
                    <div style="color:#a78bfa;font-size:20px;font-weight:700;">${S.totalKills}</div>
                </div>
                <div class="stat-card">
                    <div style="color:#8a7a6a;font-size:11px;">💰 ЗОЛОТО</div>
                    <div style="color:#fbbf24;font-size:20px;font-weight:700;">${fmt(S.gold)}</div>
                </div>
                <div class="stat-card">
                    <div style="color:#8a7a6a;font-size:11px;">🛡️ КЛАН</div>
                    <div style="color:#f5c842;font-size:20px;font-weight:700;">${S.clanName || 'Нет'}</div>
                </div>
                <div class="stat-card">
                    <div style="color:#8a7a6a;font-size:11px;">⚔️ АРМИЯ</div>
                    <div style="color:#fbbf24;font-size:20px;font-weight:700;">${S.a || 0}</div>
                </div>
            </div>
            <div style="margin-top:15px;padding:10px;background:#0a0705;border-radius:8px;">
                <div style="color:#8a7a6a;font-size:12px;">🏆 Достижения: ${S.achievements.length}/${Object.keys(ACHIEVEMENTS).length}</div>
